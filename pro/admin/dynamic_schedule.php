        ...
        <div class="modal-content" align="center">
            ...
            <div class="modal-body">
                <form action="" method="post">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                        Ticket Fee  : <input class="form-control" type="number" name="first_fee" required
                                id="">
                        </div>
                        <div class="col-sm-6">

       Plate number: <input class="form-control" type="number" name="plate_number" required
                id="">
        </div>
                    </div>
                    <div class="row">
...
if (isset($_POST['edit'])) {
    ...
    if (!isset($route_id, $bus_id, $first_fee, $plate_number, $date, $time)) {
        ...
        $conn = connect();
        $ins = $conn->prepare("UPDATE `schedule` SET `bus_id`=?,`route_id`=?,`date`=?,`time`=?,`first_fee`=?,`plate_number`=? WHERE id = ?");
        $ins->bind_param("iissiii", $bus_id, $route_id, $date, $time, $first_fee, $plate_number, $id);
        $ins->execute();
...
if (isset($_POST['bus'])) {
    $con = connect();
    $conn = $con->query("DELETE FROM schedule WHERE id = '" . $_POST['bus'] . "'");
